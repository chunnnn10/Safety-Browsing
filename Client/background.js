// 定義 API 地址（預設dev.chunnnn10.com）
const BLACKLIST_API_URL = "https://dev.chunnnn10.com/get-blacklist";
const LOG_API_URL = "https://dev.chunnnn10.com/log";
// 暫存Domains
    ...
    chrome.declarativeNetRequest.updateDynamicRules(
        {
            removeRuleIds: Array.from(
                { length: currentBlacklist.length },
                (_, i) => i + 1,
            ), // 清除舊的規則
...
chrome.webNavigation.onCompleted.addListener(
    (details) => {
        if (details.frameId === 0) {
            ...
            if (
                currentUrl.pathname.endsWith(".exe") ||
                currentUrl.pathname.endsWith(".msi")
            ) {
